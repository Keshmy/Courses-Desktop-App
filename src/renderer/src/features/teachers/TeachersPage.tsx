...
import type { TeacherDto } from '../../../../shared/types/teacher'
...
export function TeachersPage(): React.JSX.Element {
  ...
  const filtered = useMemo(() => {
    return teachers.filter((t) => {
      ...
      const haystack = [t.fullName, t.phone, t.specialization]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
      return haystack.includes(trimmedSearch)
    })
  }, [teachers, filter, trimmedSearch])
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: number) => window.api.teachers.toggleActive(id, user?.id),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="page-title">الأساتذة</h2>
          <p className="page-subtitle">سجل الأساتذة وربطهم بالمجموعات (بدون دخول للنظام)</p>
        </div>
        <Button
          className="gap-2"
          onClick={() => {
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <div className="relative mt-2">
            ...
            <Input
              className="pr-9"
              placeholder="بحث بالاسم أو الهاتف أو التخصص…"
              value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <p className="p-6 text-sm text-muted-foreground">
              {trimmedSearch ? 'لا توجد نتائج.' : 'لا يوجد أساتذة. أضف أستاذاً أولاً.'}
            </p>
          ) : (
      ...
      <TeacherFormDialog open={formOpen} onOpenChange={setFormOpen} teacher={editing} />
    </div>
  )
}
